...
library(dygraphs)
library(xts)
final.df <- read.csv("~/Documents/Programming/RaspberryPi/garage/garage.txt", header = TRUE, 
...
### Min Temp Yesterday
```{r}
gauge(round(min(ydy.df$temp.avg)), min = 40, max = 80, symbol = '°', gaugeSectors(
...
### Avg Temp Yesterday
```{r}
gauge(round(mean(ydy.df$temp.avg)), min = 40, max = 80, symbol = '°', gaugeSectors(
  success = c(65, 80), warning = c(40, 65),
  colors = c("#d6604d", "#4393c3")
))
```
...
### Max Temp Yesterday
```{r}
gauge(round(max(ydy.df$temp.avg)), min = 40, max = 80, symbol = '°', gaugeSectors(
  success = c(65, 80), warning = c(40, 65),
  colors = c("#d6604d", "#4393c3")
))
```
...
> `r paste("Last update:", final.df$date[nrow(final.df)])`
